...
This cheatsheet covers the algorithms and data structures required to solve the majority of LeetCode problems. Please [contact me](https://x.com/tlyleung) for corrections/omissions.
...
{% include figures/notes/leetcode/complexity.html %}
...
3. Implement top-down with memoization
4. Convert to bottom-up tabulation
5. Reduce space if only a limited number of previous states are needed
...
Repeatedly fix the next position by choosing a random element from the unshuffled suffix.
...
def fixed_sliding_window(nums: list[int], k: int) -> int:
    ...
    for end in range(len(nums)):
        # Expand window by adding nums[end] to state

        if end - start + 1 == k:
            # Window size is exactly k
            max_ = max(max_, contents of state)
            
            # Contract window by removing nums[start] from state
            start += 1
    
    return max_

def variable_sliding_window(nums: list[int]) -> int:
    state = # choose appropriate data structure
    start = 0
    max_ = 0
    ...
    for end in range(len(nums)):
        ...
        while state is not valid:
            # Repeatedly contract window by removing nums[start]
            # from state until it is valid again
            start += 1

        # Window state is valid
...
<section class="relative mb-4 sm:mb-8 break-inside-avoid-column overflow-hidden rounded-md bg-zinc-950/5 p-4 dark:bg-white/5" markdown="1">
<div class="absolute -top-2 right-4 size-16 text-zinc-200 dark:text-zinc-700">{% svg /assets/images/streamline/coding-apps-website-web-form-progress-1.svg width="100%" height="100%" %}</div>
...
<section class="relative mb-4 sm:mb-8 break-inside-avoid-column overflow-hidden rounded-md bg-zinc-950/5 p-4 dark:bg-white/5" markdown="1">
<div class="absolute -top-2 right-4 size-16 text-zinc-200 dark:text-zinc-700">{% svg /assets/images/streamline/column-selected-single.svg width="100%" height="100%" %}</div>
...
<span class="whitespace-nowrap rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 bg-yellow-400/20 text-yellow-700 group-data-[hover]:bg-yellow-400/30 dark:bg-yellow-400/10 dark:text-yellow-300 dark:group-data-[hover]:bg-yellow-400/15">Search: O(α(n)) (amortized)</span>
<span class="whitespace-nowrap rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 bg-green-500/15 text-green-700 group-data-[hover]:bg-green-500/25 dark:bg-green-500/10 dark:text-green-400 dark:group-data-[hover]:bg-green-500/20">Insert: O(1)</span>
<span class="whitespace-nowrap rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 bg-yellow-400/20 text-yellow-700 group-data-[hover]:bg-yellow-400/30 dark:bg-yellow-400/10 dark:text-yellow-300 dark:group-data-[hover]:bg-yellow-400/15">Space: O(n)</span>
...
<section class="relative mb-4 sm:mb-8 break-inside-avoid-column overflow-hidden rounded-md bg-zinc-950/5 p-4 dark:bg-white/5" markdown="1">
<div class="absolute -top-2 right-4 size-16 text-zinc-200 dark:text-zinc-700">{% svg /assets/images/streamline/python-logo.svg width="100%" height="100%" %}</div>
# Python
...
<section class="relative mb-4 sm:mb-8 break-inside-avoid-column overflow-hidden rounded-md bg-zinc-950/5 p-4 dark:bg-white/5" markdown="1">
<div class="absolute -top-2 right-4 size-16 text-zinc-200 dark:text-zinc-700">{% svg /assets/images/streamline/lab-flask-experiment.svg width="100%" height="100%" %}</div>
# Advanced Topics
...
Maps a permutation to its unique index (and back) using the factorial number system, by counting how many smaller unused elements precede each position.

### Hierholzer's Algorithm

Constructs an Eulerian circuit (a path that traverses every edge exactly once and ends at the starting vertex) by walking unused edges to form cycles, then splicing the cycles together into a traversal that uses every edge exactly once.

### Knuth-Morris-Pratt (KMP) String-Searching Algorithm

<span class="whitespace-nowrap rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 bg-yellow-400/20 text-yellow-700 group-data-[hover]:bg-yellow-400/30 dark:bg-yellow-400/10 dark:text-yellow-300 dark:group-data-[hover]:bg-yellow-400/15">Best: O(n+m)</span>
<span class="whitespace-nowrap rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 bg-yellow-400/20 text-yellow-700 group-data-[hover]:bg-yellow-400/30 dark:bg-yellow-400/10 dark:text-yellow-300 dark:group-data-[hover]:bg-yellow-400/15">Average: O(n+m)</span>
<span class="whitespace-nowrap rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 bg-yellow-400/20 text-yellow-700 group-data-[hover]:bg-yellow-400/30 dark:bg-yellow-400/10 dark:text-yellow-300 dark:group-data-[hover]:bg-yellow-400/15">Worst: O(n+m)</span>
...
Find $$k$$-th smallest element in an unordered list by using a partition-based approach similar to QuickSort, but only recursing on one side of the pivot.
...
To efficiently sample $$k$$ items from a stream with uniform probability, initialize a reservoir $$R$$ with the first $$k$$ items, then for each new input $$x_i$$, generate a random index $$j \in \{1, \dots, i\}$$, replacing $$R[j]$$ if $$j \leq k$$.

## Data Structures

### B-Tree

Self-balancing multi-way search tree optimized for systems that read and write large blocks of data.

### Bloom Filter

Probabilistic data structure for fast membership checks with a controlled false-positive rate.
...
Probabilistic data structure for estimating the number of distinct elements in a dataset with sublinear space.

### Interval Tree

Augmented balanced tree for querying overlapping intervals efficiently.

### $$k$$-d Tree

Space-partitioning data structure used for efficient $$k$$-nearest neighbor (KNN) searches.

### Red-Black Tree

Self-balancing binary search tree that maintains balance using color properties.

### Rope

Binary tree-based data structure designed for efficient manipulation of large strings.

### Segment Tree with Lazy Propagation and Coordinate Compression

Supports fast range queries and updates while handling sparse or large coordinate values efficiently.

### Skip List

Probabilistic data structure that allows fast search, insertion, and deletion by layering linked lists with randomized levels.
...
Determine the area of a polygon with integer vertex coordinates, given the number of integer interior ($$i$$) and boundary ($$b$$) points.
...
[^hello24]: [Data Structures and Algorithms](https://www.hellointerview.com/learn/code)
