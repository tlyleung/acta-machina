...
- [Models](#models)
  ...
  - [Tasks](#models-tasks)
  - [Image Generation Models](#models-image-generation-models)
...
Deep learning architectures that synthesise realistic or stylised images from noise, text, or existing images.

## Variational Autoencoder (VAE)

An encoder maps each input to a distribution over a latent $$z$$, and a decoder reconstructs the input from a sample of $$z$$; new images are generated by sampling $$z \sim \mathcal{N}(0, I)$$ and decoding.

- **ELBO:** trained by maximising the evidence lower bound — a reconstruction term plus a KL term that pulls the posterior toward the prior.

  $$\log p(x) \ge \mathbb{E}_{q(z \vert x)}[\log p(x \vert z)] - \mathrm{KL}(q(z \vert x) \,\Vert\, p(z))$$

- **Reparameterisation trick:** write $$z = \mu + \sigma \odot \epsilon$$ with $$\epsilon \sim \mathcal{N}(0, I)$$, moving the randomness outside the parameters so gradients can flow through the sampling step.
- **Watch out for:** posterior collapse (the decoder ignores $$z$$) and blurry samples from pixel-wise reconstruction losses; strengths are a smooth, interpolatable latent space and stable training.

---

## Generative Adversarial Network (GAN)

A generator turns noise into images while a discriminator tries to tell real from fake; they train adversarially until the generator's samples are indistinguishable from real data.

- **Sharp and fast:** produces high-quality samples in a single forward pass (e.g. StyleGAN).
- **Watch out for:** mode collapse (the generator covers only a few modes) and unstable training; Wasserstein loss with gradient penalty (WGAN-GP) and spectral normalisation help stabilise it.

---

## Diffusion Model

A fixed forward process gradually adds Gaussian noise to an image; a network learns the reverse process, so generation starts from pure noise and denoises step by step.

- **Objective:** predict the noise added at each level — equivalent to estimating the score $$\nabla_x \log p(x)$$ (denoising score matching).
- **Classifier-free guidance (CFG):** train with and without the condition, then extrapolate between the two at inference; higher guidance sharpens prompt adherence at the cost of diversity.
- **Latent diffusion:** run diffusion in a VAE's compressed latent space (Stable Diffusion) to cut compute by roughly an order of magnitude.
- **Watch out for:** slow iterative sampling (tens to hundreds of steps); DDIM, consistency models, and step distillation (LCM) trade some quality for speed.

---

## Comparison

- **VAE:** stable training and a structured latent space, but blurry samples.
- **GAN:** sharp, fast single-pass samples, but unstable and prone to mode collapse.
- **Diffusion:** best quality and diversity, but slow at inference.

Sample quality is usually measured with FID (see [Model Evaluation](#model-evaluation-metrics)).

---

## Fine-tuning & Control

- **Low-Rank Adaptation (LoRA):** freezes the pretrained weights and injects trainable low-rank matrices into each block, adapting the model cheaply.
- **DreamBooth:** fine-tunes the diffusion model on a few images of a subject so it can be generated in new contexts.
- **Textual Inversion:** learns a new embedding token for a concept from a few examples, leaving model weights frozen.
- **LyCORIS:** extends LoRA with additional conditioning and parameterisation schemes.
- **ControlNet:** conditions generation on structural inputs (edges, depth, pose) via a trainable copy of the encoder.
